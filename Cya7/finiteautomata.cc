...
bool FiniteAutomata::detectDfa() {
  for (auto it = states_with_transitions_.begin(); it != states_with_transitions_.end(); it++) {
    State to_check = *it;
    std::set<Transition> compare = to_check.get_transitions();
    if (compare.size() != input_alphabet_.size()) {
      is_dfa_ = false;
      return is_dfa_;
    }
    std::set<std::string> symbols;
    for (auto it1 = compare.begin(); it1 != compare.end(); it1++) {
      Transition check = *it1;
      symbols.insert(check.symbol_);
    }
    if (symbols.size() != input_alphabet_.size()) {
      is_dfa_ = false;
      return is_dfa_;
    }
  }
...
std::set<State> FiniteAutomata::get_states_with_transition() {
  return states_with_transitions_;
}

Alphabet FiniteAutomata::automataAlphabet() {
